...
export default class RoomStairsRight extends RoomTileEntity {
  ...
  draw() {
    ...
    for (let i = 4; i > 0; i--) {
      ...
      const second = { x: start.X - this._width / 8, y: start.Y + this._height / 8 }
      const third = { x: start.X + 24, y: start.Y + 20 }
      const fourth = { x: start.X + this._width / 2, y: start.Y + this._height / 2 }

      this.beginFill(0x989865)
      this.lineStyle(1.5, 0x8E8E5E) //2,
